...
# 1. Carica il dataset
df <- read.csv(paste0(datapath,"S2_S1_Italy_2022_trajectories_sample.csv"),stringsAsFactors = FALSE)
...
p_ndvi <- ggplot(df_ndvi, aes(
  ...
  labs(
    ...
    linetype = "Classe LC1",
    size  = "Classe LC1",
    title = "Traiettoria media mensile di NDVI per LC1 - 2022"
  ) +
  theme_minimal() +
  theme(
...
# 3) Visualizza
pdf("ndvi_trajectories_2022.pdf")
print(p_ndvi)
pdf()
...
p_vv <- ggplot(df_vv, aes(
  ...
  labs(
    ...
    linetype = "Classe LC1",
    size  = "Classe LC1",
    title = "Traiettoria media mensile di VV per LC1 - 2022"
  ) +
  theme_minimal() +
  theme(
    legend.position = "bottom",
...
p_vh <- ggplot(df_vh, aes(
  ...
  labs(
    ...
    linetype = "Classe LC1",
    size  = "Classe LC1",
    title = "Traiettoria media mensile di VH per LC1 - 2022"
  ) +
  theme_minimal() +
  theme(
    legend.position = "bottom",
