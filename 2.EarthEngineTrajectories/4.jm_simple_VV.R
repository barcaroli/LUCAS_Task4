library(tidyverse)
df <- read.csv("jm_NDVI.csv",
               stringsAsFactors = FALSE)
# df <- read.csv("S2_S1_Italy_2018_trajectories_sample.csv",
#                stringsAsFactors = FALSE)

# 2) Definizione delle colonne VV
VV_cols <- sprintf("VV_%02d", 1:12)

# 3) Calcola le traiettorie medie mensili di VV per ciascuna classe LC1
means_by_lc1 <- df %>%
  select(LC1, all_of(VV_cols)) %>%
  group_by(LC1) %>%
  summarize(across(all_of(VV_cols), ~ mean(.x, na.rm = TRUE))) %>%
  ungroup()
...
jm_cols <- paste0("JM_VV_",LETTERS[1:8])
...
write.table(df,"jm_VV.csv",sep=",",quote=F,row.names=F)
