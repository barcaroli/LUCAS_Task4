...
traj <- read.csv("trajectories_2022.csv")
...
s <- fread(paste0(datapath,"D:/Google Drive/LUCAS Copernicus/EarthEngine/data/Survey_2022_cal_wgt_2nd_phase.txt"))
b <- merge(b,s[,c("POINT_ID","SURVEY_LC1")],by="POINT_ID")
b$LC1 <- substr(b$SURVEY_LC1,1,1)
b$SURVEY_LC1 <- NULL
write.table(b,paste0(datapath,"S2_S1_Italy_2022_trajectories_dwt_sample.csv"),sep=",",quote=F,row.names=F)
