...
# Random Forest
set.seed(1234)
rf_model_C <- randomForest(target ~ ., 
                         data=train_data, 
                         importance=TRUE,
                         mtry=15,
                         nodesize=5,
                         ntree=200,
                         do.trace=TRUE)
save(rf_model_C,file=paste0(datapath,"rf_model_C.RData"))
rf_model <- rf_model_C
plot(rf_model)
...
# hist(probs)
# rf_predictions <- as.factor(ifelse(probs > 0.45, 1, 0))  # abbassa la soglia
...
rf_model_C <- randomForest(target ~ ., 
                           ...
                           ntree=200,
                           do.trace=TRUE)
save(rf_model_C,file=paste0(datapath,"rf_model_C_reduced.RData"))
rf_model <- rf_model_C
plot(rf_model)
imp_df <- as.data.frame(rf_model$importance)
imp_df$Variable <- row.names(imp_df)
