...
data$target <- as.factor(ifelse(data$LC1 == "E",1,0))
...
# Random Forest
set.seed(1234)
rf_model_E <- randomForest(target ~ ., 
                         data=train_data, 
                         importance=TRUE,
                         mtry=15,
                         nodesize=5,
                         ntree=200,
                         do.trace=TRUE)
save(rf_model_E,file=paste0(datapath,"rf_model_E.RData"))
rf_model <- rf_model_E
plot(rf_model)
...
# hist(probs)
# rf_predictions <- as.factor(ifelse(probs > 0.335, 1, 0))  # abbassa la soglia
...
#                            ntree=200,
#                            do.trace=TRUE)
# save(rf_model_E,file=paste0(datapath,"rf_model_E_reduced.RData"))
# rf_model <- rf_model_E
# plot(rf_model)
# imp_df <- as.data.frame(rf_model$importance)
# # imp_df <- importance(rf_model)
