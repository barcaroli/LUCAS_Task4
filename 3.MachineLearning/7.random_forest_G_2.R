...
data$AWEI <- 4 * (data$B3-data$B11) - (0.25*data$B8 + 2.75*data$B12)
...
# Random Forest
set.seed(1234)
rf_model_G <- randomForest(target ~ B2+B3+B4+B8+B11+B12
                         +NDWI+MNDWI*AWEI+VV+VH, 
                         data=train_data, 
                         # classwt = c("0" = 1, "1" = 100),
                         importance=TRUE,
                         # mtry=15,
...
hist(probs)
rf_predictions <- as.factor(ifelse(probs > 0.12, 1, 0))  # abbassa la soglia
