...
#-------------------------------------------------------------------------------------
# Read master
m <- fread("D:/Google Drive/LUCAS Copernicus/EarthEngine/3.MachineLearning/Italy_master_2018.csv")
summary(m)
...
write.table(mpred,"Italy_master_2018_with_predictions.csv",sep=",",quote=F,row.names=F)
library(data.table())
s <- fread("D:/Google Drive/LUCAS Copernicus/EarthEngine/data/Survey_2018_cal_wgt.txt")
...
t2 <- as.data.frame(round(prop.table(xtabs(cal_wgt~LC_predicted,data=s)),4)*100)
# t2 <- as.data.frame(round(prop.table(xtabs(wgt_2nd_phase~LC1,data=s)),4)*100)
t2

t3 <- as.data.frame(round(prop.table(xtabs(cal_wgt~LC1,data=s)),4)*100)
...
write.table(mat,"Italy_estimates_predictions_2018.csv",sep=",",quote=F,row.names=F)
