...
s <-fread(paste0(datapath,"Italy_sample_2018.csv"))
table(s$LC1)
s <- merge(m[,c("POINT_ID",c("probs_A","probs_B","probs_C","probs_D","probs_E","probs_F","probs_G","probs_H"))],
           s[,c("POINT_ID","LC1")],by="POINT_ID")
s$LC1 <- as.factor(s$LC1)
s$POINT_ID <- NULL

set.seed(42)
training_rows <- createDataPartition(s$LC1, p = 0.75, list = FALSE)
