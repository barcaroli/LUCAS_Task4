...
data$target <- as.factor(ifelse(data$LC2 == "C2",1,0))
...
# Random Forest
set.seed(1234)
rf_model_C2 <- randomForest(target ~ ., 
                         data=train_data, 
                         importance=TRUE,
                         mtry=15,
                         nodesize=5,
                         ntree=200,
                         do.trace=TRUE)
save(rf_model_C2,file=paste0(datapath,"rf_model_C2.RData"))
rf_model <- rf_model_C2
plot(rf_model)
