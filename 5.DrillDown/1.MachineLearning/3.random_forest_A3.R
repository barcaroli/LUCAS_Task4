...
data$target <- as.factor(ifelse(data$LC2 == "A3",1,0))
...
# Random Forest
set.seed(1234)
rf_model_A3 <- randomForest(target ~ ., 
                         data=train_data, 
                         importance=TRUE,
                         mtry=15,
                         nodesize=5,
                         ntree=500,
                         do.trace=TRUE)
save(rf_model_A3,file=paste0(datapath,"rf_model_A3.RData"))
rf_model <- rf_model_A3
plot(rf_model)
