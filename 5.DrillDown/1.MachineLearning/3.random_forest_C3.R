...
table(factor(data$LC2),useNA="ifany")
data$target <- as.factor(ifelse(data$LC2 == "C3",1,0))
...
# Random Forest
set.seed(1234)
rf_model_C3 <- randomForest(target ~ ., 
                         data=train_data, 
                         importance=TRUE,
                         mtry=15,
                         nodesize=5,
                         ntree=200,
                         do.trace=TRUE)
save(rf_model_C3,file=paste0(datapath,"rf_model_C3.RData"))
rf_model <- rf_model_C3
plot(rf_model)
...
hist(probs)
rf_predictions <- as.factor(ifelse(probs > 0.22, 1, 0))  # abbassa la soglia
