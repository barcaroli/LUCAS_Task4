...
rf_model <- randomForest(LC2 ~ ., 
                         ...
                         ntree=500,
                         do.trace=TRUE)
save(rf_model,file=paste0(datapath,"ensemble_model_C.RData"))
...
# write.table(mpred,paste0(datapath,"Italy_master_2018_with_predictions_C.csv"),sep=",",quote=F,row.names=F)

s2 <- fread(paste0(datapath,"Survey_2018_cal_wgt.txt"))
s2 <- s2[s2$NUTS0_16 == "IT",]
s2 <- s2[substr(s2$land_cover,1,1) == "C",]
...
write.table(mat,paste0(datapath,"Italy_estimates_predictions_2018_C.csv"),sep=",",quote=F,row.names=F)
