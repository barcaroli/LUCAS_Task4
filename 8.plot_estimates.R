# ------------------------------------------------------------
# Grafici A–H: sample vs master (2018–2022) + obs 2018/2022
# (etichette e asse Y con 2 decimali)
# ------------------------------------------------------------
library(tidyverse)
library(scales)
...
make_plot <- function(df, classe) {
  ...
  y_lower <- max(0, min(y_all) - pad)   # niente più soglia che riporta a 0
  ...
  ggplot() +
    ...
    labs(
      title = paste0("Classe: ", classe, " — Sample vs Master (2018–2022) + Obs 2018/2022"),
      ...
      x = "Anno", y = "Quota", color = NULL, linetype = NULL
    ) +
...
# pdf("Estimates 2018 - 2022.pdf", width = 8.5, height = 5.6)
# for (cl in classes) {
#   p <- make_plot(df, cl)
#   if (!is.null(p)) print(p)
# }
