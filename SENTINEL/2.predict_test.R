...
sample <- fread(paste0(datapath,"Italy_sample_2018_reduced_aux_completo_rain_fourier.csv"))
fa <- names(sample)[vapply(sample, is.factor, logical(1))]
ch <- names(sample)[vapply(sample, is.character, logical(1))]
for (cc in ch) sample[[cc]] <- factor(sample[[cc]])
