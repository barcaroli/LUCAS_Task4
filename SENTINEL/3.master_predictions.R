...
#------------------------------------------------------------------------------
setwd("D:/Google Drive/LUCAS Copernicus/EarthEngine/SENTINEL/workflow")
...
#-------------------------------------------------------------------------------------
# 2018
m <- fread(paste0(datapath,"Italy_master_2018_reduced_aux_completo_rain_fourier.csv"))
c <- cor(m[,c(5:68)])
corrplot(c, method = "color", type = "upper", tl.cex = 0.8)

pred <- mod$predict_newdata(m)   
table(pred$response)
...
#-------------------------------------------------------------------------------------
# 2019
m <- fread(paste0(datapath,"Italy_master_2019_reduced_aux_completo_rain_fourier.csv"))
...
#-------------------------------------------------------------------------------------
# 2020
m <- fread(paste0(datapath,"Italy_master_2020_reduced_aux_completo_rain_fourier.csv"))
pred <- mod$predict_newdata(m)   # new_df = solo feature A00..A63 (+eventuali extra)
pred$set_threshold(th_mlr3)
...
#-------------------------------------------------------------------------------------
# 2021
m <- fread(paste0(datapath,"Italy_master_2021_reduced_aux_completo_rain_fourier.csv"))
pred <- mod$predict_newdata(m)   # new_df = solo feature A00..A63 (+eventuali extra)
pred$set_threshold(th_mlr3)
...
#-------------------------------------------------------------------------------------
# 2022
m <- fread(paste0(datapath,"Italy_master_2022_reduced_aux_completo_rain_fourier.csv"))
pred <- mod$predict_newdata(m)   # new_df = solo feature A00..A63 (+eventuali extra)
pred$set_threshold(th_mlr3)
